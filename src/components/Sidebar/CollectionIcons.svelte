	...
	<div class="h-auto w-full flex flex-row flex-wrap gap-3">
		{#each collection.icons as icon}
			<div class="relative icon-container">
				...
				<button type="button" class="h-fit w-24 flex flex-col items-center gap-3 p-2 rounded-md transition-all cursor-pointer" onclick={() => selectUserIcon(icon, collection)} aria-label={`Select icon ${icon.label}`} >
