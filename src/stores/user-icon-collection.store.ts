...
import { UserIconCollectionIndexedDBService } from "../services/user-icon-collection-indexeddb.service"; // Import the service
...
function createCollectionsStore() {
    ...
    UserIconCollectionIndexedDBService.fetchList().then(collections => {
    ...
    async function addCollection(newCollection: UserIconCollection) {
        newCollection.id = await UserIconCollectionIndexedDBService.create(newCollection);
    ...
    return {
        ...
        addIcon: async (collectionId: string, icon: UserIcon) => {
            ...
            if (icon.id === UUID.empty) {
                icon.id = await UserIconCollectionIndexedDBService.addUserIcon(icon, collectionId);
            }
        ...
        removeIcon: async (collectionId: string, iconId: string) => {
            ...
            await UserIconCollectionIndexedDBService.update(collection);
        },
        ...
        removeCollection: async (collectionId: string) => {
            ...
            await UserIconCollectionIndexedDBService.delete(collectionId);
        },
        set
    };
}
